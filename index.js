...
function sanitizeReportHtml(text) {
  ...
  // verbatim: the panel splits the report on `<!-- REPORT_ZH -->`. Sanitize
  // everything, then restore EVERY escaped marker to canonical form. Keeping
  // all occurrences live (not just the first) lets the client pick the true
  // boundary: the panel chooses the first marker whose IMMEDIATELY-following
  // text is Chinese, so quoted markers inside the English or Chinese halves
  // are skipped and only the real English→Chinese boundary is used.
  return String(text)
    .replace(DANGEROUS, (m) => m.replace(/</g, '&lt;').replace(/>/g, '&gt;'))
    .replace(/&lt;!-- REPORT_ZH -->/g, '<!-- REPORT_ZH -->');
}
  ...
  async function runScanLlm(info, scan, state, reportDir, { signal }) {
    ...
    try {
      ...
      if (isInsideReports(reportsRoot, reportDir)) {
        // report.md stores the model's raw output (sanitized) with its own
        // bilingual marker. Attribution footers are appended CLIENT-SIDE after
        // the panel splits the report (en half gets the English footer, zh
        // half gets the Chinese footer), so the server never has to guess the
        // marker boundary — a model that quotes the marker string inside a
        // finding used to misplace a server-side footer and mix the languages.
        writeFileSync(join(reportDir, 'report.md'), sanitizeReportHtml(report), { mode: 0o600 });
